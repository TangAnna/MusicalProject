 ...
 * @date :${DATA} 11:11
 */
public class BrowseArea extends BaseRelativeLayout<List<MusicalModel>> {
    private RecyclerView mRecyclerView;
    private List<MusicalModel> mData;
    private CommonAdapter mAdapter;

    public BrowseArea(Context context) {
        super(context);
    }

    public BrowseArea(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public BrowseArea(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    protected void initView(Context context, AttributeSet attrs, int defStyleAttr) {
        ((HomeItemTitle) findViewById(R.id.title_area)).setData("Browse by Genre");
        mRecyclerView = findViewById(R.id.rv_body);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));

        mData = new ArrayList<>();
        mRecyclerView.setAdapter(mAdapter = new CommonAdapter<MusicalModel>(getContext(), R.layout.item_browse, mData) {
            @Override
